  ...
  <div class="w-100 p-2">
    <div class="row position-relative">
      ...
      <div class="col-3 ps-2 position-absolute top-0 end-0" v-if="keep.creator.id == account.id">
        <div class="pt-1">
          <button @click="deleteVaultKeep(keep.vaultKeepId)" class="btn btn-outline-danger border-0 rounded-4 py-1 px-2"><i class="mdi mdi-delete-forever text-shadow-shallow"></i></button>
        </div>
      </div>
    </div>
  </div>
